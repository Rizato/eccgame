...
/**
 * Calculate distance between private keys for practice mode progress
 */
export function getPrivateKeyDistance(currentPrivateKey: bigint, targetPrivateKey: bigint): number {
  // Calculate the absolute difference between private keys
  const diff =
    currentPrivateKey > targetPrivateKey
      ? currentPrivateKey - targetPrivateKey
      : targetPrivateKey - currentPrivateKey;

  // Normalize to a percentage (0-100%)
  // Smaller differences = higher progress
  const maxDistance = CURVE_N / 2n; // Maximum meaningful distance
  const normalizedDistance = Number(diff) / Number(maxDistance);

  // Convert to progress percentage (closer = higher percentage)
  return Math.max(0, (1 - normalizedDistance) * 100);
}

/**
 * Estimate current private key from operation history (best effort reconstruction)
 */
export function estimatePrivateKeyFromOperations(
  operations: Array<{
  ...
  try {
    // Attempt to reconstruct the private key from scalar operations only
    return reconstructPrivateKey(
      operations.filter(op => op.type === 'multiply' || op.type === 'divide')
    );
  } catch {
    // If reconstruction fails, estimate based on point distance
    // This is a rough approximation when point addition/subtraction is involved
    const currentDiff = getPointDistance(startingPoint, targetPoint);
    if (currentDiff.isInfinity) {
      return 1n; // Points are the same, private key is 1
    }

    // Use x-coordinate as a rough private key estimate
    // This is not mathematically accurate but gives some progress feedback
    return currentDiff.x % CURVE_N;
  }
}

/**
 * Get Euclidean distance between two points (alternative distance measure)
 */
export function getEuclideanDistance(p1: ECPoint, p2: ECPoint): number {
  if (p1.isInfinity || p2.isInfinity) {
    return Infinity;
  }

  // Simple Euclidean distance in the coordinate space
  const dx = Number((p1.x - p2.x) % CURVE_P);
  const dy = Number((p1.y - p2.y) % CURVE_P);

  // Normalize to prevent overflow and get relative distance
  const xDiff = dx / Number(CURVE_P);
  const yDiff = dy / Number(CURVE_P);
