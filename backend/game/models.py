...
class Challenge(models.Model):
    ...
    public_key = models.ForeignKey(
        PublicKey, on_delete=models.CASCADE, related_name="public_key"
    )
...
class Guess(models.Model):
    ...
    uuid = models.UUIDField(default=uuid.uuid4, editable=False, blank=True, unique=True)
    submitted_key = models.ForeignKey(
        PublicKey, on_delete=models.CASCADE, related_name="submitted_key"
    )
    challenge = models.ForeignKey(
        Challenge, on_delete=models.CASCADE, to_field="uuid", related_name="challenge"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
